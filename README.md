...
- **管理端**：分支与版本的增删改查（弹窗式交互 + 分页列表），侧边栏导航布局
- **用户端**：水平时间轴视图（全屏独立页面，暗色主题，无管理框架）
...
管理端和用户端共用同一 SPA，通过路由切换布局：

- **管理端**（`/`、`/branches`、`/versions`）：左侧深色侧边栏 + 右侧内容区
- **用户端**（`/timeline`）：全屏无框架，`App.vue` 检测路由后跳过侧边栏布局，直接渲染全屏视图

## 页面结构

| 页面 | 路由 | 说明 |
|------|------|------|
| 仪表盘 | `http://localhost:8080` | 管理端首页：统计卡片 + 分支树 + 最近版本 |
| 分支管理 | `/#/branches` | 管理端：分页表格 + 弹窗式新建/编辑/详情 |
| 版本管理 | `/#/versions` | 管理端：筛选 + 分页表格 + 弹窗式新建/编辑/详情 |
| **时间轴** | **`/#/timeline`** | **用户端独立页面：分支名固定左侧、缩放、拖拽、斜排标签、一年默认视图** |
...
### 时间轴：独立全屏页面（2026-06-17 重写）
`App.vue` 检测 `/timeline` 路由后跳过侧边栏，`router-view` 占满视口。暗色主题（`#1a1b2e`），SVG 元素配色适配。

**双层面板布局**：左侧 150px 分支名固定面板 + 右侧 SVG 水平可滚动区域，两者垂直滚动同步。

**核心功能**：
- 分支名固定在左侧，水平滚动时不移动
- 默认展示最新版本向前一年，更早数据向左滚动查看
- 鼠标滚轮缩放（0.3x–8x），以鼠标位置为中心，视觉元素大小不变
- 时间轴始终铺满视口（ResizeObserver + 动态分支间距）
- 版本名 35° 斜排（左下→右上），减少重叠
- 鼠标左键拖拽平移
